...
case class GoogleSubConfig(
                            pullInterval: FiniteDuration,
                            acknowledgeBatchBuffer: AcknowledgeBatchBufferConfig)
...
case class GoogleSub(client: GoogleSubClient, logger: Logger, config: GoogleSubConfig) {
  ...
  val messages: Observable[ReceivedMessage] = {
    Observable.interval(config.pullInterval) >> {
      Observable.fromTask(client.token >>= client.pull).map(_.receivedMessages.getOrElse(Seq.empty)).filter(_.nonEmpty) >>=
  ...
  def acknowledgeBatch(ackId: AckId): Observable[AcknowledgeRequest] =
    Observable(ackId).bufferTimedAndCounted(config.acknowledgeBatchBuffer.time, config.acknowledgeBatchBuffer.maxCount) >>= acknowledgeBatch
