...
case class CircuitBreakerConfig(maxFailures: Int, resetTimeout: FiniteDuration, exponentialBackoffFactor: Double, maxResetTimeout: FiniteDuration) {
...
case class GoogleSubHttpClientConfig(uri: Uri, project: String, subscription: String, pullMaxMessages: Int, client: ClientConfig, circuitBreaker: CircuitBreakerConfig)

//TODO - F[_] could be used instead of IO
//TODO - pass the CB
case class GoogleSubHttpClient(httpClient: Client[IO], time: Time, logger: Logger, config: GoogleSubHttpClientConfig) extends GoogleSubClient {
  ...
  private def acknowledgeRequest(request: AcknowledgeRequest): IO[Request[IO]] =
    Method.POST(
      config.uri.withPath("/v1/projects/" |+| config.project |+| "/subscriptions/" |+| config.subscription |+| ":acknowledge"),
      request
    )
  ...
  private def tokenRequest(request: OAuthRequest): IO[Request[IO]] = IO(request.encode(config.client.privateKey)).flatMap { encodedRequest ⇒
    Method.POST(
  ...
  private def newTokenWithCircuitBreaker(cb: TaskCircuitBreaker): Task[AccessToken] = cb.protect {
    for {
      ...
      oauthResponse ← Task.fromIO(tokenRequest(OAuthRequest.from(config.client.email, now, expiresAt)) >>= httpClient.expect[OAuthResponse])
